...
{
    public class SqlTableCheckDayanimSinifi
    {
        ...
        {
            string TabloSorgusu = "SELECT name FROM ElkonScada.sys.tables where name= 'DayanimSinifi' " ;
            ...
            {
                string CreateTstr = "CREATE TABLE DayanimSinifi" +
               "(SiraNo INTEGER IDENTITY(1,1) PRIMARY KEY,DayanimSinifi CHAR(100), Aciklama CHAR(150)," +
            "Durumu CHAR(50))";
        ...
        {
            DataAdapter = new SqlDataAdapter("Select ROW_NUMBER() OVER(ORDER BY SiraNo) AS No,* from DayanimSinifi", DatabaseConn);
            dt = new DataTable();
            DataAdapter.Fill(dt);
            return dt;
        }
