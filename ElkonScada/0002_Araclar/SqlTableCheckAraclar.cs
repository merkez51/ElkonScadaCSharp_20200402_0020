...
{
    public class SqlTableCheckAraclar
    {
        ...
        {
            string TabloSorgusu = "SELECT name FROM ElkonScada.sys.tables where name= 'Araclar' ";                             
            ...
            {
                string CreateTstr = "CREATE TABLE Araclar" +
               "(SiraNo INTEGER IDENTITY(1,1) PRIMARY KEY,AracPlakaNo CHAR(100), FirmaAdi CHAR(150)," +
            "SurucuAdiSoyadi CHAR(150),Kapasite CHAR(50),AracTipi CHAR(150),AracMarkasi CHAR(100),Durumu CHAR(50))";
        ...
        {
            DataAdapter = new SqlDataAdapter("Select ROW_NUMBER() OVER(ORDER BY SiraNo) AS No,* from Araclar", DatabaseConn);
            dt = new DataTable();
            DataAdapter.Fill(dt);
            return dt;
        }
