...
{
    public class SqlTableCheckSantiyeler
    {
        ...
        {
            string TabloSorgusu = "SELECT name FROM ElkonScada.sys.tables where name ='Santiyeler' ";
            ...
            {
                string CreateTstr = "CREATE TABLE Santiyeler" +
               "(SiraNo INTEGER IDENTITY(1,1) PRIMARY KEY,SantiyeAdi CHAR(150), TelefonFaks CHAR(100)," +
            "Adres CHAR(150),FirmaAdi CHAR(150),Mesafe CHAR(50),Aciklama CHAR(150),Durumu CHAR(50))";
        ...
        {
            DataAdapter = new SqlDataAdapter("Select ROW_NUMBER() OVER(ORDER BY SiraNo) AS No,* from Santiyeler", DatabaseConn);
            dt = new DataTable();
            DataAdapter.Fill(dt);
            return dt;
        }
