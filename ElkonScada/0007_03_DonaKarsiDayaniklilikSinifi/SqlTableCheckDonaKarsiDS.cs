...
{
    public class SqlTableCheckDonaKarsiDS
    {
        ...
        {
            string TabloSorgusu = "SELECT name FROM ElkonScada.sys.tables where name= 'DonaKarsiDayaniklilikSinifi' ";
            ...
            {
                string CreateTstr = "CREATE TABLE DonaKarsiDayaniklilikSinifi" +
               "(SiraNo INTEGER IDENTITY(1,1) PRIMARY KEY,DonaKarsiDayaniklilikSinifi CHAR(100), Aciklama CHAR(150)," +
            "Durumu CHAR(50))";
        ...
        {
            DataAdapter = new SqlDataAdapter("Select ROW_NUMBER() OVER(ORDER BY SiraNo) AS No,* from DonaKarsiDayaniklilikSinifi", DatabaseConn);
            dt = new DataTable();
            DataAdapter.Fill(dt);
            return dt;
        }
