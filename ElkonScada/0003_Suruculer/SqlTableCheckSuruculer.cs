...
{
    public class SqlTableCheckSuruculer
    {
        ...
        {
            string TabloSorgusu = "SELECT name FROM ElkonScada.sys.tables where name= 'Suruculer' ";
            ...
            {
                string CreateTstr = "CREATE TABLE Suruculer" +
               "(SiraNo INTEGER IDENTITY(1,1) PRIMARY KEY,SurucuAdiSoyadi CHAR(100), Adres CHAR(150)," +
            "TelefonFaks CHAR(100),Aciklama CHAR(150),Durumu CHAR(50))";
        ...
        {
            DataAdapter = new SqlDataAdapter("Select ROW_NUMBER() OVER(ORDER BY SiraNo) AS No,* from Suruculer", DatabaseConn);
            dt = new DataTable();
            DataAdapter.Fill(dt);
            return dt;
        }
