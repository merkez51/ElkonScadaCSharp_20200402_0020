...
{
    public class SqlTableCheckCevreselEtki
    {
        ...
        {
            string TabloSorgusu = "SELECT name FROM ElkonScada.sys.tables where name= 'CevreselEtki' ";
            ...
            {
                string CreateTstr = "CREATE TABLE CevreselEtki" +
               "(SiraNo INTEGER IDENTITY(1,1) PRIMARY KEY,CevreselEtki CHAR(100), Aciklama CHAR(150)," +
            "Durumu CHAR(50))";
        ...
        {
            DataAdapter = new SqlDataAdapter("Select ROW_NUMBER() OVER(ORDER BY SiraNo) AS No,* from CevreselEtki", DatabaseConn);
            dt = new DataTable();
            DataAdapter.Fill(dt);
            return dt;
        }
