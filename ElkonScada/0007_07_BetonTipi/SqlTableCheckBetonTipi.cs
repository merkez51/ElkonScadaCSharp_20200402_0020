...
{
    public class SqlTableCheckBetonTipi
    {
        ...
        {
            string TabloSorgusu = "SELECT name FROM ElkonScada.sys.tables where name= 'BetonTipi' ";
            ...
            {
                string CreateTstr = "CREATE TABLE BetonTipi" +
               "(SiraNo INTEGER IDENTITY(1,1) PRIMARY KEY,BetonTipi CHAR(100), Aciklama CHAR(150)," +
            "Durumu CHAR(50))";
        ...
        {
            DataAdapter = new SqlDataAdapter("Select ROW_NUMBER() OVER(ORDER BY SiraNo) AS No,* from BetonTipi", DatabaseConn);
            dt = new DataTable();
            DataAdapter.Fill(dt);
            return dt;
        }
