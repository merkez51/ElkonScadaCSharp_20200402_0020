...
{
    public class SqlTableCheckSuyaKarsiDS
    {
        ...
        {
            string TabloSorgusu = "SELECT name FROM ElkonScada.sys.tables where name= 'SuyaKarsiDayaniklilikSinifi' ";
            ...
            {
                string CreateTstr = "CREATE TABLE SuyaKarsiDirencSinifi" +
               "(SiraNo INTEGER IDENTITY(1,1) PRIMARY KEY,SuyaKarsiDirencSinifi CHAR(100), Aciklama CHAR(150)," +
            "Durumu CHAR(50))";
        ...
        {
            DataAdapter = new SqlDataAdapter("Select ROW_NUMBER() OVER(ORDER BY SiraNo) AS No,* from SuyaKarsiDirencSinifi", DatabaseConn);
            dt = new DataTable();
            DataAdapter.Fill(dt);
            return dt;
        }
