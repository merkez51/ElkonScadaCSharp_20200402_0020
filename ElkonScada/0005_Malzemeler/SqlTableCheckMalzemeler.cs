...
{
    public class SqlTableCheckMalzemeler
    {
        ...
        {
            string TabloSorgusu = "SELECT name FROM ElkonScada.sys.tables where name= 'Malzemeler' ";
            ...
            {
                string CreateTstr = "CREATE TABLE Malzemeler" +
        ...
        {
            DataAdapter = new SqlDataAdapter("Select ROW_NUMBER() OVER(ORDER BY SiraNo) AS No,* from Malzemeler", DatabaseConn);
            dt = new DataTable();
            DataAdapter.Fill(dt);
            return dt;
        }
