...
{
    public class SqlTableCheckKlorurIcerigiSinifi
    {
        ...
        {
            string TabloSorgusu = "SELECT name FROM ElkonScada.sys.tables where name= 'KlorurIcerigiSinifi' ";
            ...
            {
                string CreateTstr = "CREATE TABLE KlorurIcerigiSinifi" +
               "(SiraNo INTEGER IDENTITY(1,1) PRIMARY KEY,KlorurIcerigiSinifi CHAR(100), Aciklama CHAR(150)," +
            "Durumu CHAR(50))";
        ...
        {
            DataAdapter = new SqlDataAdapter("Select ROW_NUMBER() OVER(ORDER BY SiraNo) AS No,* from KlorurIcerigiSinifi", DatabaseConn);
            dt = new DataTable();
            DataAdapter.Fill(dt);
            return dt;
        }
