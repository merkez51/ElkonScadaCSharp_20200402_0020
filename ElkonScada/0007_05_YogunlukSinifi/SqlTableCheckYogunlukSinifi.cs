...
{
    public class SqlTableCheckYogunlukSinifi
    {
        ...
        {
            string TabloSorgusu = "SELECT name FROM ElkonScada.sys.tables where name= 'YogunlukSinifi' ";
            ...
            {
                string CreateTstr = "CREATE TABLE YogunlukSinifi" +
               "(SiraNo INTEGER IDENTITY(1,1) PRIMARY KEY,YogunlukSinifi CHAR(100), Aciklama CHAR(150)," +
            "Durumu CHAR(50))";
        ...
        {
            DataAdapter = new SqlDataAdapter("Select ROW_NUMBER() OVER(ORDER BY SiraNo) AS No,* from YogunlukSinifi", DatabaseConn);
            dt = new DataTable();
            DataAdapter.Fill(dt);
            return dt;
        }
